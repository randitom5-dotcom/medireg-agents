...
| 文档 | 说明 |
| --- | --- |
| [MediReg 项目架构图](docs/MediReg项目架构图.md) | 项目整体架构、智能体结构、数据流和目录职责 |
| [MediReg 数据库结构报告](docs/MediReg数据库结构报告.md) | `medireg_db` 表结构、字段说明和样例数据 |
| [医疗器械注册知识库数据预处理与建库方案](docs/医疗器械注册知识库数据预处理与建库方案.md) | 数据清洗、分类、去重、分流和建库方案 |
| [医疗器械注册知识库搭建与召回方案报告](docs/医疗器械注册知识库搭建与召回方案报告.md) | RAGFlow、MySQL 和混合召回设计 |
| [医疗器械注册知识库验证用例](docs/医疗器械注册知识库验证用例.md) | RAGFlow 召回、Agent 问答和端到端验证用例 |
| [医疗器械注册知识库多用户数据隔离设计报告](docs/医疗器械注册知识库多用户数据隔离设计报告.md) | 公共、工作区、用户私有数据隔离设计 |
| [外包团队接口对接说明](docs/外包团队接口对接说明.md) | 外包前端或外部系统对接 `/api/v1` 的接口说明 |
| [可行性分析报告](reports/可行性分析报告.md) | 项目整体可行性、架构、部署和阶段计划 |
| [医疗器械注册资料分析报告](reports/医疗器械注册资料分析报告.md) | 示例注册资料分析报告 |
...
├── examples/                       # 示例脚本、RAGFlow SDK demo、测试文档和旧示例素材
