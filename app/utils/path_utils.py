...
def resolve_path(filename: str, session_dir: Optional[str] = None) -> str:
    ...
    # Avoid duplicating the active session path while preserving subdirectories
    # such as reports/foo.md.
    if session_name in parts:
        session_index = parts.index(session_name)
        trailing_parts = parts[session_index + 1 :]
        if trailing_parts:
            return str(session_path / Path(*trailing_parts))
        return str(session_path)

    if parts and parts[0] == "output":
        trailing_parts = parts[1:]
        if trailing_parts and trailing_parts[0] == session_name:
